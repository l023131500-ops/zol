...
router.patch('/devices/:id', requireAuth, (req, res) => {
  const { device, error } = getOwnedDevice(req, req.params.id);
  if (error) return res.sendStatus(error);
  const { name, homeUrl, allowedHost } = req.body || {};
  db.prepare('UPDATE devices SET name = COALESCE(?, name), home_url = COALESCE(?, home_url), allowed_host = COALESCE(?, allowed_host) WHERE id = ?')
    .run(name ?? null, homeUrl ?? null, allowedHost ?? null, device.id);
  ...
  // Tell the device to re-pull its config.
  issueCommand(fresh, 'update_config', { homeUrl: fresh.home_url, allowedHost: fresh.allowed_host }, req.user.id);
...
router.post('/enrollments', requireAuth, (req, res) => {
  ...
  const { name, homeUrl, allowedHost } = req.body || {};
  if (!homeUrl) return res.status(400).json({ error: 'נדרשת כתובת אתר (URL) שהמכשיר יציג' });
  let host = allowedHost;
  if (!host) { try { host = new URL(homeUrl).host; } catch { return res.status(400).json({ error: 'כתובת אתר לא תקינה' }); } }
  const code = codeGen();
  const expires = new Date(Date.now() + 14 * 24 * 3600 * 1000).toISOString();
  const info = db.prepare('INSERT INTO enrollments (owner_id, code, name, home_url, allowed_host, expires_at) VALUES (?, ?, ?, ?, ?, ?)')
    .run(req.user.id, code, name || null, homeUrl, host, expires);
  logEvent(null, req.user.id, 'enrollment_created', code);
  res.json({ enrollment: db.prepare('SELECT id, code, name, home_url, allowed_host, used, expires_at FROM enrollments WHERE id = ?').get(info.lastInsertRowid) });
});
...
function publicDevice(d) {
  return {
    ...
    allowedHost: d.allowed_host, homeUrl: d.home_url, status: d.status, online: !!d.online,
